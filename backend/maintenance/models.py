...
class ServiceCatalog(models.Model):
    ...
    # Pricing Information (Canadian dollars)
    price = models.DecimalField(
        ...
        help_text="Service price in CAD (must be under CAD 100)"
    )
    ...
    def formatted_price(self):
        """Return formatted Canadian-dollar price."""
        if self.price:
            return f"CAD ${self.price:,.2f}"
...
class ServiceProvider(models.Model):
    ...
    SERVICE_TYPES = [
        ('cleaner', 'Cleaner'), ('plumber', 'Plumber'), ('electrician', 'Electrician'),
        ('carpenter', 'Carpenter'), ('painter', 'Painter'), ('hvac', 'HVAC Technician'),
        ('locksmith', 'Locksmith'), ('gardener', 'Gardener'), ('general', 'General Maintenance'),
        ('other', 'Other'),
    ]
...
class MaintenanceRequest(models.Model):
    ...
    CATEGORY_CHOICES = [
        ...
        ('appliance', 'Appliance'), ('structural', 'Structural'), ('cleaning', 'Cleaning'),
        ('painting', 'Painting'), ('carpentry', 'Carpentry'), ('locksmith', 'Locksmith'),
        ('gardening', 'Gardening'), ('pest_control', 'Pest Control'),
    ...
    service_catalog = models.ForeignKey(ServiceCatalog, on_delete=models.SET_NULL, null=True, blank=True, related_name='service_bookings', help_text="Pre-defined service from catalog")
    ...
    booking_type = models.CharField(max_length=20, choices=[('emergency', 'Emergency'), ('scheduled', 'Scheduled')], default='scheduled')
    admin_confirmed_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='confirmed_service_bookings', help_text="Admin who confirmed the service request")
    admin_confirmed_at = models.DateTimeField(null=True, blank=True)
    admin_rejection_reason = models.TextField(blank=True, help_text="Reason if service request was rejected by admin")

    QUOTE_STATUS_CHOICES = [
        ('none', 'No Quote Yet'), ('pending', 'Pending Landlord Review'), ('approved', 'Approved'),
...
class MaintenanceSchedule(models.Model):
    ...
    def update_next_due_date(self):
        ...
        if not self.last_completed:
            return
        if self.frequency == 'weekly': self.next_due_date = self.last_completed + timedelta(weeks=1)
        elif self.frequency == 'bi_weekly': self.next_due_date = self.last_completed + timedelta(weeks=2)
        elif self.frequency == 'monthly': self.next_due_date = self.last_completed + relativedelta(months=1)
        elif self.frequency == 'quarterly': self.next_due_date = self.last_completed + relativedelta(months=3)
        elif self.frequency == 'semi_annual': self.next_due_date = self.last_completed + relativedelta(months=6)
        elif self.frequency == 'annual': self.next_due_date = self.last_completed + relativedelta(years=1)
